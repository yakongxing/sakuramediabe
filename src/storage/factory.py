...
def storage_for(namespace: str):
    ...
    return WebDAVStorageBackend(settings.storage.webdav_base_url, namespace, username=settings.storage.username, password=settings.storage.password, root_prefix=settings.storage.root_prefix, verify_tls=settings.storage.verify_tls, timeout=timeout, final_visibility_retry_delays=settings.storage.webdav_final_visibility_retry_seconds, publication_concurrency_limit=settings.storage.webdav_publication_max_workers, temp_cleanup_interval_seconds=settings.storage.webdav_temp_cleanup_interval_seconds, temp_cleanup_age_seconds=settings.storage.webdav_temp_cleanup_age_seconds, temp_cleanup_max_deletes=settings.storage.webdav_temp_cleanup_max_deletes)
